<template>
    ...
    <el-table :data="list" style="width: 100%">
        ...
        <el-table-column label="规格值" #default="scope">
            <div v-for="(item1, index1) in scope.row.specValue" :key="index1"
...
const editShow = row => {
    productSpec.value = {
        ...
        specValue: row.specValue.map(item => ({
            key: item.key,
...
// 保存修改
const update = async () => {
    console.log('productSpec.value', productSpec.value)
    ...
    console.log('productSpecClone.specValue before: ', productSpecClone.specValue)
    console.log(typeof productSpecClone.specValue)
    ...
    productSpecClone.specValue.forEach(item => {
        ...
        if (typeof item.valueList === 'string') {
            ...
            console.log('convert string to array: ', item.valueList)
            item.valueList = item.valueList.split(',')
        }
    })
    productSpecClone.specValue = JSON.stringify(productSpecClone.specValue)
    console.log('productSpecClone.specValue after: ', productSpecClone.specValue)
    console.log(typeof productSpecClone.specValue)
