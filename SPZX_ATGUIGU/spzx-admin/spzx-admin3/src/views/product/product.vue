<template>
    ...
    <el-table :data="list" style="width: 100%">
        ...
        <el-table-column label="操作" align="center" width="200" #default="scope">
            ...
            " type="warning" size="small" @click="updateStatus(scope.row.id, 1)" style="padding: 5px; margin-left: 5px;">
                上架
            </el-button>
            <el-button v-if="
                scope.row.auditStatus == 1 &&
            ...
            <el-collapse v-model="activeNames">
                <el-collapse-item title="商品基本信息" name="productBaseInfo">
                    ...
                    <el-form-item label="轮播图">
                        <el-upload v-model:file-list="fileList" action="http://localhost:8501/admin/system/uploadFile"
                ...
                <el-collapse-item title="商品SKU信息" name="skuInfo">
                    <el-form-item label="选择规格">
                        <el-select :disabled="product.id != '' && product.id != undefined" v-model="product.specValue"
                            ...
                            <el-option v-for="item in specsList" :key="item.specValue" :label="item.specName"
                        ...
                        <el-table :data="product.productSkuList" border style="width: 100%">
                            ...
                            <el-table-column label="图片" #default="scope" width="80">
                                <el-upload class="avatar-uploader"
                                    action="http://localhost:8501/admin/system/uploadFile" :show-file-list="false"
                                    :on-success="(response, uploadFile, fileList) =>
                                            handleSkuSuccess(
                                                response,
                                                uploadFile,
                                                fileList,
                ...
                <el-collapse-item title="商品详情信息" name="productDetails">
                    <el-form-item label="详情图片">
                        <el-upload v-model:file-list="detailsFileList"
                            action="http://localhost:8501/admin/system/uploadFile" list-type="picture-card" multiple
...
import { FindAllProductSpecs } from '@/api/productSpecs.js'
...
// 分类级联选择器数据发生改变以后的事件处理函数
const categoryChange = async () => {
    console.log('categoryChange title', title.value)
...
const fetchProductSpecs = async () => {
    const { data } = await FindAllProductSpecs()
    specsList.value = data
}
...
const getById = async id => {
    ...
    console.log('getById - product.value = ', product.value)
