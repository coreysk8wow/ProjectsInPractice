...
export const brand = mysqlTable(
	...
	(table) => [primaryKey({ columns: [table.id], name: "brand_id" })]
);
...
export const categoryBrand = mysqlTable(
	"category_brand",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
	...
	(table) => [primaryKey({ columns: [table.id], name: "category_brand_id" })]
);

export const couponInfo = mysqlTable(
	"coupon_info",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
		...
		conditionAmount: decimal("condition_amount", {
			precision: 10,
			scale: 2,
		})
			.default("0.00")
	...
	(table) => [primaryKey({ columns: [table.id], name: "coupon_info_id" })]
);

export const couponRange = mysqlTable(
	"coupon_range",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
	...
	(table) => [primaryKey({ columns: [table.id], name: "coupon_range_id" })]
);

export const couponUser = mysqlTable(
	"coupon_user",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
		...
		getTime: datetime("get_time"),
		usingTime: datetime("using_time"),
		usedTime: datetime("used_time"),
		expireTime: datetime("expire_time"),
		createTime: timestamp("create_time")
			.defaultNow()
			.notNull(),
	...
	(table) => [primaryKey({ columns: [table.id], name: "coupon_user_id" })]
);

export const orderInfo = mysqlTable(
	"order_info",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
		...
		originalTotalAmount: decimal("original_total_amount", {
			...
			.default("0.00")
			.notNull(),
		feightFee: decimal("feight_fee", { precision: 10, scale: 2 })
			.default("0.00")
			.notNull(),
		...
		deliveryTime: datetime("delivery_time"),
	...
	(table) => [primaryKey({ columns: [table.id], name: "order_info_id" })]
);
...
export const orderLog = mysqlTable(
	"order_log",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
		orderId: bigint("order_id", { mode: "number" }),
	...
	(table) => [primaryKey({ columns: [table.id], name: "order_log_id" })]
);

export const orderStatistics = mysqlTable(
	"order_statistics",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const paymentInfo = mysqlTable(
	"payment_info",
	{
		...
		callbackTime: datetime("callback_time"),
...
export const productAttr = mysqlTable(
	"product_attr",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
	...
	(table) => [primaryKey({ columns: [table.id], name: "product_attr_id" })]
);
...
export const productDetails = mysqlTable(
	...
	(table) => [primaryKey({ columns: [table.id], name: "product_details_id" })]
);

export const productSku = mysqlTable(
	"product_sku",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
	...
	(table) => [primaryKey({ columns: [table.id], name: "product_sku_id" })]
);

export const productSpec = mysqlTable(
	"product_spec",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
	...
	(table) => [primaryKey({ columns: [table.id], name: "product_spec_id" })]
);

export const productUnit = mysqlTable(
	"product_unit",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
		name: varchar({ length: 255 }),
	...
	(table) => [primaryKey({ columns: [table.id], name: "product_unit_id" })]
);
...
export const sysLoginLog = mysqlTable(
	"sys_login_log",
	{
		...
		accessTime: datetime("access_time"),
		createTime: timestamp("create_time")
			.defaultNow()
			.onUpdateNow()
			.notNull(),
		updateTime: timestamp("update_time").onUpdateNow(),
		isDeleted: tinyint("is_deleted").default(0).notNull(),
	},
	(table) => [primaryKey({ columns: [table.id], name: "sys_login_log_id" })]
);
...
export const sysOperLog = mysqlTable(
	"sys_oper_log",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
	...
	(table) => [primaryKey({ columns: [table.id], name: "sys_oper_log_id" })]
);
...
export const sysRole = mysqlTable(
	...
	(table) => [primaryKey({ columns: [table.id], name: "sys_role_id" })]
);
...
export const sysUser = mysqlTable(
	...
	(table) => [primaryKey({ columns: [table.id], name: "sys_user_id" })]
);
...
export const userAddress = mysqlTable(
	...
	(table) => [primaryKey({ columns: [table.id], name: "user_address_id" })]
);

export const userBrowseHistory = mysqlTable(
	"user_browse_history",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
		userId: bigint("user_id", { mode: "number" }).notNull(),
...
export const userCollect = mysqlTable(
	"user_collect",
	{
		id: bigint({ mode: "number" }).autoincrement().notNull(),
		userId: bigint("user_id", { mode: "number" }).notNull(),
...
export const userInfo = mysqlTable(
	...
	(table) => [primaryKey({ columns: [table.id], name: "user_info_id" })]
);
