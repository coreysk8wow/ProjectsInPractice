...
import { ISysUserRoleService, ISysUserService } from "@/services/interfaces/system-service-intf";
import { SysUserRoleServiceImpl, SysUserServiceImpl } from "@/services/system-service-impl";
...
userRouter.get("/findByPage/:pageNum/:pageSize", async (req, res) => {
	...
	const usersPageInfo: PageInfo<ISysUser> = await sysUserService.findByPage(
		sysUserReq,
		pageNum,
		pageSize
	);
